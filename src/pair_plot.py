...
def main():
    ...
    for i, row_col in enumerate(numeric_cols):
        for j, col_col in enumerate(numeric_cols):
            ...
            if i == j:
                for house in houses:
                    grades = df[df['Hogwarts House'] == house][row_col].dropna()
                    ax.hist(grades, bins=20, alpha=0.5, color=house_colors[house], 
            ...
            # Rest
                ...
                for house in houses:
                    ...
                    ax.scatter(grades[col_col], grades[row_col], 
                               alpha=0.5, s=15, color=house_colors[house], edgecolors='none')
            
            # Clean up the axes (hide ticks inside the grid, keep outer labels)
            # if i < n_features - 1:
            #     ax.set_xticks([])
            # else:
            #     ax.set_xlabel(col_col, rotation=90, fontsize=8)
                
            # if j > 0:
            #     ax.set_yticks([])
            # else:
            #     ax.set_ylabel(row_col, rotation=0, fontsize=8, ha='right')

    # Create a custom legend for the figure
    handles = [plt.Line2D([0], [0], marker='o', color='w', 
                          markerfacecolor=house_colors[h], markersize=10) for h in houses]
    fig.legend(handles, houses, loc='upper right', title='Hogwarts House', fontsize=12)   
    ...
    useless_features = [
        ...
        'Defense Against the Dark Arts' # Redundant (correlates with Astronomy)
    ]
